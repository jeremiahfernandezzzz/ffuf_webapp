...
class Container implements DIContainerInterface{
    ...
    public function make($class){
        ...
        if (!isset($this->closureCache[$className])) {
            ...
            $params = $constructor ? $this->resolveMethodParameters($classInfo['constructor'], $className,
                '__construct') : null;
            $setters = array();
            $properties = $classInfo['properties'];
            if (!$refClass->isInstantiable()) {
                $this->resolving = array();
                throw new InjectionException("Class/Interface " . $className . ' is not instantiable!',
                    InjectionException::CLASS_NOT_INSTANTIABLE);
            }
    ...
    protected function resolveMethodParameters(array $parameters, $class, $method){
        ...
        foreach ($parameters as $paramName => $pInfo) {
            if (isset($this->sharedClassParameters[$class][$method]) &&
                array_key_exists($paramName, $this->sharedClassParameters[$class][$method])
            ) {
            ...
            if (!isset($pInfo['class']) || null === $pInfo['class']) {
                $this->resolving = array();
                throw new InjectionException('No value for param ' . $paramName,
                    InjectionException::MISSING_REQUIRED_PARAM);
            }
            if(isset($this->classAliases[$class][$method][$paramName])){
                $pInfo['class'] = $this->classAliases[$class][$method][$paramName];
            }
     ...
     * @return self
     */
    public function addParameterAliases($class, $method, array $aliases){
        foreach ($aliases as $parameter => $alias) {
            $this->classAliases[$class][$method][$parameter] = $alias;
        }
        return $this;
    }
}
