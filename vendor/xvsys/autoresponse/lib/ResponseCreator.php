...
class ResponseCreator implements EventSubscriberInterface{
    ...
    public function onKernelException(GetResponseForExceptionEvent $e){
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if ($exClass == 'xvsys\validator\UserInputException' ||
                $exClass == 'a15l\cmp\symfony\validator\container\exception\UserInputException'
            ) {
                $errorMsgs[] = $ex->getMessage();
            }
            if ($exClass == 'xvsys\validator\ValidationException' ||
                $exClass == 'a15l\cmp\symfony\validator\container\exception\ValidationException'
            ) {
